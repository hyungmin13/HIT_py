...
#%% valid_pred_list_total 는 모든 시간 [i] 에 대해 중심으로 부터의 거리 [j] 에 따른 속도, 압력의 예측값
    ...
    train_pos_from_center = train_pos_unnorm - np.array([0,0.05,0.05,0.05]).reshape(-1,4)
    ...
    valid_pos_from_center = valid_pos_unnorm[1,:,:,:,:].reshape(-1,4) - np.array([0,0.05,0.05,0.05]).reshape(-1,4)
    ...
    for j in tqdm(range(50), desc="Valid data binning"):
        ...
        for i in range(len(valid_indexes)):
            ...
            pos_sub_unnorm_list.append(valid_pos_unnorm[129**3*j:129**3*(j+1),:][valid_indexes[i],:])
            pos_sub_list.append(valid_data['pos'][129**3*j:129**3*(j+1),:][valid_indexes[i],:])
